...
function crc16(body: Uint8Array): [number, number] {
  let crc = 0xFFFF;
  for (const b of body) {
    crc ^= b << 8;
    for (let i = 0; i < 8; i++) {
...
function buildCmd(body: number[]): Uint8Array {
...
function buildTimeSyncCmd(): Uint8Array {
  const now = new Date();
...
async function writeWithResponse(
  char: BluetoothRemoteGATTCharacteristic,
  data: Uint8Array,
): Promise<void> {
  if (typeof (char as any).writeValueWithResponse === 'function') {
    await (char as any).writeValueWithResponse(data);
  } else {
    await char.writeValue(data as unknown as ArrayBuffer);
  }
}

// ── UI helpers ─────────────────────────────────────────────────────────────
type LogLevel = 'default' | 'hr' | 'info' | 'warn';

function log(msg: string, level: LogLevel = 'default'): void {
...
function setStatus(text: string, cls: 'idle' | 'connecting' | 'connected' | 'error'): void {
  const el = document.getElementById('statusEl')!;
  el.className = 'status ' + cls;
  document.getElementById('statusText')!.textContent = text;
}

function setHR(bpm: number | null, source = ''): void {
...
function setActivity(steps: number, cal: number, hrCache: number): void {
...
function checkReadiness(bytes: Uint8Array): void {
...
function onCmdChar(event: Event): void {
...
function onHRChar(event: Event): void {
  const v     = (event.target as BluetoothRemoteGATTCharacteristic).value!;
  const bytes = new Uint8Array(v.buffer);
...
function onHRStd(event: Event): void {
  const v     = (event.target as BluetoothRemoteGATTCharacteristic).value!;
  const bytes = new Uint8Array(v.buffer);
  if (bytes.length < 2) return;
  ...
  if (!liveHRActive) setHR(hr, 'std · 2A37');
}
...
async function connectToDevice(device: BluetoothDevice): Promise<void> {
  ...
  } catch { log('2A37 not available', 'info'); }
  ...
  } catch { log('FEA1 not available under FEE7', 'info'); }
...
async function tryAutoReconnect(): Promise<void> {
  ...
  try {
    ...
    const ring = (devices as BluetoothDevice[]).find(d => d.name?.includes('TK5'));
    if (ring) {
